...
- (void)viewDidLoad
{
  ...
  //  [self plistSpooler];
    ...
    groupByCategoryLabel.font = [UIFont fontWithName:@"Dosis-Regular" size:17.0f];
}
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row == searchData.count-1){//search
        ++pageno;
        [self nextPage];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(searchVariable==0)
    {
        ...
        if (cell == nil) {
            ...
            categoryNameLabel.textColor =[UIColor colorWithRed:58/255.0 green:56/255.0 blue:48/255.0 alpha:1];
            categoryNameLabel.font = [UIFont fontWithName:@"Dosis-Regular" size:19.f];
            ...
            categoryGroups.font = [UIFont fontWithName:@"Dosis-Regular" size:19.f];
...
- (void)nextPage{
    ...
    NSString *postData = [NSString stringWithFormat:@"search_data=%@&user_id=%@&current_page=%d",searchText,appUserId,pageno];
    NSLog(@"request %@",postData);
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/scripts/search_group_user.php",gupappUrl]]];
    [request setHTTPMethod:@"POST"];
    [request setValue:@"application/x-www-form-urlencoded; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
...
//                NSLog(@"app uid %@",appUserId);
                if (![result[@"id"]isEqualToString:appUserId]){
                    ...
                    CGSize size =[self calculateHeight:[[datav objectForKey:@"description"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
...
//            FilterButton = [[UIButton alloc] initWithFrame: CGRectMake(0, 0, 60.0f, 30.0f)];
//            [FilterButton setTitle:@"Filter" forState:UIControlStateNormal];
//            [FilterButton addTarget:self action:@selector(setFilterTable:) forControlEvents:UIControlEventTouchUpInside];
//            [FilterButton setTitleColor:[UIColor colorWithRed:255.0/255.0 green:207.0/255.0 blue:13.0/255.0 alpha:1] forState:UIControlStateNormal];
//            FilterButton.titleLabel.font = [UIFont fontWithName:@"Dosis-Bold" size:20];
//            
//            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:FilterButton];
...
//                        NSString *insertPublicGroup=[NSString stringWithFormat:@"insert into groups_public (group_server_id, location_name, category_name, added_date,is_favourite, group_name,group_type, group_pic,group_description,total_members) values ('%@','%@','%@','%@','%d','%@','%@','%@','%@','%@')",[searchData objectAtIndex:rows][@"id"],groups[@"location_name"],groups[@"category_name"],groups[@"creation_date"],1,[groups[@"group_name"] stringByReplacingOccurrencesOfString:@"'" withString:@"''"],groups[@"group_type"],groups[@"group_pic"],[groups[@"group_description"] stringByReplacingOccurrencesOfString:@"'" withString:@"''"],groups[@"member_count"]];
//                        NSLog(@"query %@",insertPublicGroup);
//                        [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:insertPublicGroup];
                         [[self appDelegate]._chatDelegate buddyStatusUpdated];
                    ...
                    {
//                        NSString *checkIfGroupExists=[NSString stringWithFormat:@"select * from group_invitations where group_id=%@",[searchData objectAtIndex:rows][@"id"]];
//                        BOOL groupExistOrNot=[[DatabaseManager getSharedInstance]recordExistOrNot:checkIfGroupExists];
//                        if (groupExistOrNot) {
//                            NSString *updateQuery=[NSString stringWithFormat:@"update  group_invitations set group_id = '%@', group_name = '%@', group_pic = '%@', group_type ='%@' where group_id = '%@' ",[searchData objectAtIndex:rows][@"id"],[[searchData objectAtIndex:rows][@"name"] normalizeDatabaseElement],[searchData objectAtIndex:rows][@"display_pic"],[searchData objectAtIndex:rows][@"type"],[searchData objectAtIndex:rows][@"id"]];
//                            [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:updateQuery];
//                        }
//                        else
//                        {
//                            NSString *insertQuery=[NSString stringWithFormat:@"insert into group_invitations (group_id, group_name, group_pic, group_type) values ('%@','%@','%@','%@')",[searchData objectAtIndex:rows][@"id"],[[searchData objectAtIndex:rows][@"name"] normalizeDatabaseElement],[searchData objectAtIndex:rows][@"display_pic"],[searchData objectAtIndex:rows][@"type"]];
//                            NSLog(@"query %@",insertQuery);
//                            [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:insertQuery];
//                        }
